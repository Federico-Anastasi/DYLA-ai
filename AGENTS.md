...
**Codex Sonnet** through the cloud. You cannot tell which from inside, and you do not
...
Skills live in `.Codex/skills/`. Each writes or updates exactly one document, and each
...
# Making Dyla your own — for Codex, in the repo

*Everything above is the system prompt of the agent that runs inside the app. This last
section is for a different reader: the coding assistant (you, Codex) that someone
...
**Add a skill.** A skill is one folder under `.Codex/skills/<name>/` with a single
`SKILL.md` — plain instructions, no code: read these files, ask these questions (in the
```questions block, see `.Codex/prompts/questions_format.md`), write that JSON, wait for
